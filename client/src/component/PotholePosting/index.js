	...
	return (
		<Card>
			<Card.Title className="post-header">
				<img src={logo} alt="user icon" className="post-logo" />{' '}
				<h1 className="location-name">@{post.location}</h1>
			</Card.Title>
			...
			<footer className="blockquote-footer footer-date">
				Posted on <Moment format="YYYY/MM/DD">{post.date}</Moment> {'  '}by{' '}
				<cite title="Source Title">{post.name}</cite>
				{'    '}
				<Button className="float-right notes-button">
					View Discussion <span className="comment-count">{post.comments.length}</span>
				</Button>
				<button
					type="button"
					className="btn btn-light ml-4 middle-buttons"
